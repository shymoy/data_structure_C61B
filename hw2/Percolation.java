...
public class Percolation {
    ...
    public Percolation(int N) {
        ...
        openSiteNumber = 0;
        this.size = N;
        uf = new WeightedQuickUnionUF(N * N + 1); //检查是不是full的
        ufHelper = new WeightedQuickUnionUF(N * N + 2);//检查是不是连通的
        topSite = N * N;
        bottomSite = N * N + 1;
    ...
    private void connectIfopen(int row1 , int col1, int row2, int col2) {
        if (row1 >=0 && row1 < size && col1 >=0 && col1 < size && isOpen(row1, col1) && isOpen(row2, col2)) {
