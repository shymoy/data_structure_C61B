...
public class MergeSort {
    ...
    public static <Item extends Comparable> Queue<Item> mergeSort(
        ...
        while (tmp.size() != 1) {
            ...
            while (!(tmp.isEmpty())) {
                Queue<Item> q = tmp.dequeue();
                Queue<Item> p = tmp.isEmpty() ? new Queue<>() : tmp.dequeue();
                tmpp.enqueue(mergeSortedQueues(q, p));
            }
            tmp = tmpp;
        }
        return tmp.dequeue();
    ...
    public static void main(String[] args) {
        Queue<String> student = new Queue<>();
        student.enqueue("Bob");
        student.enqueue("Carl");
        student.enqueue("Alice");
        student.enqueue("Dan");
        Queue<String> orderedStudent = mergeSort(student);
